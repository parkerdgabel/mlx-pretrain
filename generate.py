...
def main():
    ...
    greedy_score_index = 0
    while greedy_score_index < len(output):
        if output[greedy_score_index][1] < greedy_score:
            break
        greedy_score_index += 1
    num_to_display = 4
    for i, (beam, score) in enumerate(output[:num_to_display]):  # Limit to top 4 beams for printing
        """
        Print each beam generated by the beam search with its score.
        """
        if i == greedy_score_index:
            # Highlight the greedy output in the printed beams
            print(f"Greedy Output (Score: {greedy_score:.3f}):     {trainer.tokenizer.detokenize(greedy_output)}")
        print(f"Generated Beam #{i + 1} (Score: {score:.3f}): {trainer.tokenizer.detokenize(beam)}")
    if greedy_score_index >= num_to_display:
        print(f"Greedy Output (Score: {greedy_score:.3f}):     {trainer.tokenizer.detokenize(greedy_output)}")
